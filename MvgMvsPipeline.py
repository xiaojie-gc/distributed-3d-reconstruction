...
if not CAMERA_SENSOR_DB_DIRECTORY:
    CAMERA_SENSOR_DB_DIRECTORY = "/home/zxj/zxj/openMVG/src/software/SfM" + "/../../openMVG/exif/sensor_width_database"


    PRESET = {'SEQUENTIAL': [0, 1, 2, 3], #, 10, 11, 12, 13
          'GLOBAL': [0, 1, 2, 4, 9, 10, 11, 12, 13],
          'MVG_SEQ': [0, 1, 2, 3, 5, 6, 7],
          'MVG_GLOBAL': [0, 1, 2, 4, 5, 6, 7],
          'MVS_SGM': [14, 15],
          'OPENMVG': [1, 2, 6],
          'OPENMVS': [9, 10, 11, 12, 13],
          'GOLD': [1, 2, 6, 9, 10, 11, 12, 13],
          #'OPENMVG_BG': [1, 2, 6],
          'MVG_MVS': [9],
          'DensifyPointCloud': [10]
          }
    ...
    def __init__(self):
        self.steps_data = [
            ...
            ["Compute features",             # 1
             ...
             ["-i", "%matches_dir%/sfm_data.json", "-o", "%matches_dir%", "-m", "SIFT", "-n", "8", "-p", "HIGH"]], #HIGH
            ["Compute matches",              # 2
            ...
            ["Structure from Known Poses",   # 6
             ...
             ["-i", "%matches_dir%/sfm_data.json", "-m", "%matches_dir%", "-o", "%reconstruction_dir%/sfm_data.bin"]],
            ...
            ["Densify point cloud",          # 10
             ...
             ["scene.mvs", "--dense-config-file", "Densify.ini", "--resolution-level", "0", "--max-resolution", "%resolution%", "--min-resolution", "320", "-w", "%mvs_dir%", "--tasks", "%tasks%", "--task-number", "%task_number%", "--do-fuse", "%do_fuse%"]],
    ...
    def apply_conf(self, conf):
        ...
        for s in self.steps_data:
            ...
            for o in s[2]:
                ...
                co = co.replace("%sfm_data.bin%", conf.sfm)
                co = co.replace("%camera_file_params%", conf.camera_file_params)
                co = co.replace("%tasks%", conf.tasks)
                co = co.replace("%do_fuse%", conf.do_fuse)
                co = co.replace("%task_number%", conf.task_number)
                co = co.replace("%resolution%", conf.resolution)
                o2.append(co)
            s[2] = o2
...
PARSER.add_argument('--sfm', default="sfm_data.bin",
                    help="the path to sfm.bin")
PARSER.add_argument('--mvs_dir',
                    help="the path to mvs")
PARSER.add_argument('--tasks', default="0123456",
                    help="task IDs")
PARSER.add_argument('--task_number', default="6",
                    help="number of tasks")
PARSER.add_argument('--do_fuse', default="1",
                    help="do_fuse")
PARSER.add_argument('--verbose', default="no",
                    help="verbose")
PARSER.add_argument('--resolution', default="1920",
                    help="verbose")
...
CONF.mvs_dir = os.path.join(CONF.output_dir, CONF.mvs_dir)
...
if CONF.verbose != "no":
    ...
    for i in range(len(time_file["timeList"])):
        if time_file["timeList"][i]["id"] == CONF.verbose:
            inx = i
            break

    if inx is None:
        time_file["timeList"].append({
	              "id": CONF.verbose,
		       "steps": {}
        })
        inx = len(time_file["timeList"]) - 1
...
for cstep in CONF.steps:
    ...
    if not DEBUG:
        ...
        try:
            ...
            pStep = subprocess.Popen(cmdline, stdout=DEVNULL, stderr=STDOUT) #
            pStep.wait()
            # print(time.time()-start_step)
    ...
    #if CONF.verbose!= "no":
    #    time_file["timeList"][inx]["steps"][STEPS[cstep].info] = round(time.time() - startTime, 3)

#printout("# Pipeline end #", effect=INVERSE)
if CONF.verbose!= "no":
    # print(json.dumps(time_file["timeList"][inx], indent=4))
